...
ADD CONSTRAINT default_registration_date DEFAULT GETDATE() FOR registration_date;

CREATE TABLE CONTRACTORBANK(
	contractor_bank INTEGER,
	contractor_INTEGER INTEGER,
	account_INTEGER VARCHAR(40),
	account_name VARCHAR(40),
	bban VARCHAR(40),
	CONSTRAINT contrabank_pk PRIMARY KEY(contractor_bank, contractor_INTEGER, account_INTEGER),
	CONSTRAINT contractor_bank_fk FOREIGN KEY(contractor_bank) REFERENCES bank(bank_code),
	CONSTRAINT contrabank_contractor_INTEGER_fk FOREIGN KEY(contractor_INTEGER) REFERENCES CONTRACTOR(contractor_code)
);
...
CREATE TABLE CONTRACTORQUALIFICATION(
	...
	CONSTRAINT contra_id_fk FOREIGN KEY(contractor_id) REFERENCES CONTRACTOR(contractor_code)
);
...
DROP TABLE WSCRIPTALLOCATION;
CREATE TABLE WSCRIPTALLOCATION( -- SCRIPT ALLOCATION FOR GABECE
	schoolId VARCHAR(10),
	subjectMarked VARCHAR(10),
	examinerID INTEGER,
	numOfScript INTEGER ,
	...
	CONSTRAINT WsubjectMarked_fk FOREIGN KEY(subjectMarked) REFERENCES GABECESUBJECT(gsubject_code),
...
ALTER TABLE VEHICLEREFUND
ADD contractor INTEGER;
ALTER TABLE VEHICLEREFUND
ADD CONSTRAINT vehicleRefund_contractor FOREIGN KEY(contractor)	REFERENCES CONTRACTOR(contractor_code);
...
CREATE TABLE WMEETINGDETAILS( --MEETINGS FOR GABECE, THIS WILL CAPTURE ALL TYPES OF MEETINGS
	...
	CONSTRAINT WmeetingDetail_WMcode FOREIGN KEY(WMcode) REFERENCES MEETINGS(meeting_code),
